...
const EventDetails = ({ isConnected }) => {
  ...
  const purchaseTicket = async () => {
    ...
    try {
      ...
      // Use a dedicated ticket image instead of the event image
      // This ensures tickets have a consistent, ticket-specific appearance
      const ticketImageUrl = getTicketFallbackImage(event, 0); // Use 0 as default ticket ID for preview
      console.log(`Using ticket image: ${ticketImageUrl}`);
    ...
    <div className="event-details-container">
      ...
      <div className="event-info">
        ...
        <div className="event-tickets">
          ...
          {isUpcoming(event.date) && event.ticketsSold < event.maxTickets && (
            <div className="ticket-preview" style={{ marginTop: '1rem' }}>
              <p><strong>Ticket Preview:</strong></p>
              <img
                src={getTicketFallbackImage(event, 0)}
                alt="Ticket Preview"
                style={{
                  width: '100%',
                  maxWidth: '200px',
                ...
                onError={(e) => {
                  e.target.onerror = null;
                  // Use cultural image for ticket preview if this is a cultural event
                  if (event.name.toLowerCase().includes('cultural') ||
                    ...
                    e.target.src = 'https://img.freepik.com/free-vector/realistic-golden-ticket-template_52683-35936.jpg';
                  }
                }}
              />
            </div>
